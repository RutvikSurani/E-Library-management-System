<?php session_start(); ?>
  ...
  <body>
  	...
  	<?php
		...
		$query = $conn->query("SELECT * FROM book  where b_type='$a' ORDER BY upload_on DESC");
		if ($query->num_rows > 0) {
			while ($row = $query->fetch_assoc()) {
				$imageURL = 'php/upload/ScienceFiction/' . $row["i_name"];
				$FileURL = 'php/upload/ScienceFiction/' . $row["f_name"];
				$BookName = $row["b_name"];
